...
import {
  Search, Filter, Download, Loader2, BriefcaseIcon,
      ...
      <Tabs defaultValue="all" className="space-y-4">
        ...
        {(['all', 'active', 'closed'] as const).map(tab => {
          const filtered = applications
            .filter(a => {
              if (tab === 'active') return ['applied', 'screening', 'interviewing', 'offered'].includes(a.status);
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-3 mb-3">
                            <div className="w-10 h-10 rounded-full bg-cyan-400/10 flex items-center justify-center shrink-0">
                              <span className="text-cyan-400 font-bold text-sm">
                                {app.candidate_name.split(' ').map(n => n[0]).join('').slice(0, 2)}
                              </span>
                            </div>
                            <div className="min-w-0">
                              ...
                              <p className="text-gray-400 text-sm truncate">{app.candidate_email}</p>
                            </div>
                          </div>
                          ...
                          {app.skills && (
                            <div className="flex flex-wrap gap-1.5">
                              {app.skills.split(',').slice(0, 5).map(skill => (
                                <span
                                  key={skill.trim()}
                        ...
                        <div className="flex flex-col items-end gap-2 shrink-0">
                          ...
                          {app.experience_years > 0 && (
                            <span className="text-xs text-gray-400">
                              {app.experience_years} yr{app.experience_years !== 1 ? 's' : ''} exp
                            </span>
                          )}
