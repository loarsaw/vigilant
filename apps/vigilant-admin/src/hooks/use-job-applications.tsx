...
export function useJobApplications(filters: JobApplicationsFilters = {}) {
  ...
  const {
    ...
    error,
    refetch,
  } = useQuery<JobApplicationsResponse, Error>({
  ...
  const approveApplicationAsync = (id: string, notes?: string) =>
    updateStatusMutation.mutateAsync({ id, payload: { status: 'offered', notes } })

  const rejectApplicationAsync = (id: string, notes?: string) =>
    updateStatusMutation.mutateAsync({ id, payload: { status: 'rejected', notes } })
